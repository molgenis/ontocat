...
public final class FileOntologyService extends AbstractOntologyService implements OntologyService {
	...
	public FileOntologyService(URI uriOntology, String ontologyAccession) {
		...
		ontoAccessions.add(ontology.getOntologyID().toString().replaceFirst("^<", "").replace(">$",
				""));
		ontoAccessions.add(loader.getManager().getOntologyDocumentIRI(ontology).toString());
	...
	public List<String> getSynonyms(String ontologyAccession, String termAccession)
		...
		List<String> temp;
		Map<String, List<String>> annots = getAnnotations(ontologyAccession, termAccession);

		temp = annots.get(synonymSlot);
		if (temp != null)
			result.addAll(dropQuotes(temp));

		temp = annots.get("synonym");
		if (temp != null)
			result.addAll(dropQuotes(temp));

		temp = annots.get("alternative_term");
		if (temp != null)
			result.addAll(dropQuotes(temp));
	...
	private String getOntologyAccession(OWLClass cls) throws OntologyServiceException {
		...
		Matcher matcher = pattern.matcher(cls.toStringID());
		if (matcher.find()) {
	...
	public List<OntologyTerm> getChildren(String ontologyAccession, String termAccession)
		...
		if (cls == null)
			return Collections.EMPTY_LIST;
		for (OWLClassExpression desc : cls.getSubClasses(ontology)) {
			if (!desc.isAnonymous()) {
	...
	public List<OntologyTerm> getParents(String ontologyAccession, String termAccession)
		...
		if (cls == null)
			return Collections.EMPTY_LIST;
		for (OWLClassExpression desc : cls.getSuperClasses(ontology)) {
			if (!desc.isAnonymous()) {
				list.add(getTerm(desc.asOWLClass()));
			}
		}
	...
	public String makeLookupHyperlink(String termAccession) {
		try {
			return getOwlClass(termAccession).toStringID();
		} catch (OntologyServiceException e) {
			e.printStackTrace();
			return null;
		}
	...
	private String getFragment(OWLClass cls) {
		return getFragment(cls.getIRI().toURI());
	}
