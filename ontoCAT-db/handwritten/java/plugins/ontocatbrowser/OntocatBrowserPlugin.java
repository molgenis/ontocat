...
//			this.qs = locator.getOntologyQuery();
	...
	private ArrayList<String> findSearchTermPath() {
		...
		try {
			...
			//	LinkedHashMap<String, String> possibleParents = new LinkedHashMap(qs.getTermParents(termAcc, screenModel.getSelectedOntology()));
				...
				List<uk.ac.ebi.ontoapi.OntologyTerm> on2 = os.getTerm(termAcc).getParents();
				for(uk.ac.ebi.ontoapi.OntologyTerm o : on2){
				...
				if (possibleParents.size() == 1) {
					...
					for (String tryParent : possibleParents.keySet()) {
						...
						on2 = os.getTerm(tryParent).getChildren();
						for(uk.ac.ebi.ontoapi.OntologyTerm o : on2){
	...
	private void search(Tuple request) {
		try {
			if (screenModel.getSearchSpace().equals("this")) {
				...
				List<uk.ac.ebi.ontoapi.OntologyTerm> on2 = os.searchOntology(screenModel.getSearchThis(), screenModel.getSelectedOntology());
				for(uk.ac.ebi.ontoapi.OntologyTerm o : on2){
				...
				HashMap<String, String> resultSet = new HashMap<String, String>();
				List<uk.ac.ebi.ontoapi.OntologyTerm> on2 = os.searchAll(screenModel.getSearchThis());
				for(uk.ac.ebi.ontoapi.OntologyTerm o : on2){
					resultSet.put(o.getAccession(), o.getLabel());
				}
				if (resultSet.size() != 0) {
	...
	private int ChangeName(Database db, Tuple request) {
		...
		try {
			...
			ontocatdb.OntologyTerm ot = db.find(ontocatdb.OntologyTerm.class, new QueryRule("termAccession", Operator.EQUALS, screenModel.getExplored().getAccession())).get(0);
	...
	private void RefreshDB(Database db) {
		try {
			...
			List<ontocatdb.OntologyTerm> allTerms = db.find(ontocatdb.OntologyTerm.class);
			for (ontocatdb.OntologyTerm ot : allTerms) {
	...
	private void ExploreFromDB(Database db, boolean justBeenSaved) {
		...
		try {
			...
			ontocatdb.OntologyTerm ot = db.find(ontocatdb.OntologyTerm.class, findTerm).get(0);
			...
			if (accs.equals(ot.getTerm())) {
				...
				Map<String, String[]> os1 = os.getTerm(accs).getAnnotations();
				...
				map = new HashMap<String, String>();
				os1 = os.getTerm(accs).getRelations();
				for(String key : os1.keySet()){
						String[] valArr = os1.get(key);
						String concatVal = "";
						for(String subVal : valArr){
				...
				map = new HashMap<String, String>();
				List<uk.ac.ebi.ontoapi.OntologyTerm> on2 = os.getTerm(accs).getParents();
				for(uk.ac.ebi.ontoapi.OntologyTerm o : on2){
				...
				et.setInvestigation(db.find(ontocatdb.Investigation.class, new QueryRule("id", Operator.EQUALS, ot.getInvestigation())).get(0));
				System.out.println("et.getInvestigation().getName(): " + et.getInvestigation().getName());
				
				// Set graph URI
				String uri = "http://www.ebi.ac.uk/ontology-lookup/displayTermImage.do?termId=" + accs + "&termName=" + ot.getTerm() + "&ontologyName=" + onto + "&graphType=root";
	...
	private void Remove(Database db) {
		try {
			...
			if (screenModel.getSelectedStoredTerm() != -1) {
				...
				QueryRule findSource = new QueryRule("id", QueryRule.Operator.EQUALS, ot.getTermSource());
	...
	private int storeOntologyTerm(Database db, Tuple request) {
		...
		if (screenModel.getExplored().getTerm() != null) {
			try {
				...
				List<ontocatdb.OntologyTerm> tryToFind = db.find(ontocatdb.OntologyTerm.class, new QueryRule("termAccession", Operator.EQUALS, screenModel.getExplored().getAccession()));
				if (tryToFind.size() == 0) {
	...
	private boolean BrowseForth(boolean rootTerm, boolean lastTerm) {
		...
		try {
			...
			if (rootTerm == true) {
				...
				List<uk.ac.ebi.ontoapi.OntologyTerm> on2 = os.getRootTerms(bt);
				for(uk.ac.ebi.ontoapi.OntologyTerm o : on2){
			...
			if (rootmap.size() == 0) {
				...
				List<uk.ac.ebi.ontoapi.OntologyTerm> on2 = os.getTerm(bt).getChildren();
				for(uk.ac.ebi.ontoapi.OntologyTerm o : on2){
	...
	private boolean BrowseForthOLD(boolean rootTerm, boolean lastTerm) {
		...
		try {
			...
			if (rootTerm == true) {
				...
				List<uk.ac.ebi.ontoapi.OntologyTerm> on2 = os.getRootTerms(screenModel.getSelectedBrowserTerm());
				for(uk.ac.ebi.ontoapi.OntologyTerm o : on2){
					map.put(o.getAccession(), o.getLabel());
				}
			...
			if (map.size() == 0) {
				...
				map = new LinkedHashMap<String, String>();
				List<uk.ac.ebi.ontoapi.OntologyTerm> on2 = os.getTerm(screenModel.getSelectedBrowserTerm()).getChildren();
				for(uk.ac.ebi.ontoapi.OntologyTerm o : on2){
					map.put(o.getAccession(), o.getLabel());
				}
				...
				Map<String, String[]> os1 = os.getTerm(screenModel.getSelectedBrowserTerm()).getRelations();
				for(String key : os1.keySet()){
						String[] valArr = os1.get(key);
						String concatVal = "";
						for(String subVal : valArr){
	...
	public void checkIfTermsExistInDB(Database db) {
		try {
			...
			List<ontocatdb.OntologyTerm> matchingTerms = db.find(ontocatdb.OntologyTerm.class, accessionMatch);
			...
			for (String key : screenModel.getBrowserTerms().keySet()) {
				boolean match = false;
				for (ontocatdb.OntologyTerm term : matchingTerms) {
	...
	public void reload(Database db) {
		...
		try {
			...
			try {
				...
				List<ontocatdb.OntologyTerm> storedTerms = db.find(ontocatdb.OntologyTerm.class);
